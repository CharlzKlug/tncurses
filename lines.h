...
static int InsertLn_Cmd(ClientData cdata, Tcl_Interp *interp,
			int objc, Tcl_Obj *const objv[]) {
  CHECK_ARGUMENTS(1, "wrong # args: should be \"insertln\"");
  ...
  Tcl_AppendResult(interp, "error occured while insertln", NULL);
  return TCL_ERROR;
}
